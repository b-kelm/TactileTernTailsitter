...
const int ch1Pin = 17; //throttle //BEN
const int ch2Pin = 15; //ail //BEN
const int ch3Pin = 16; //ele //BEN
const int ch4Pin = 20; //rudd //BEN
const int ch5Pin = 21; //gear (throttle cut)
const int ch6Pin = 22; //aux1 (free aux channel)
const int PPM_Pin = 23; // BEN
//OneShot125 ESC pin outputs:
const int m1Pin = 25; // Right Motor - BEN
const int m2Pin = 24; // Left Motor - BEN
const int m3Pin = 2;
const int m4Pin = 3;
const int m5Pin = 4;
const int m6Pin = 5;
//PWM servo or ESC outputs:
const int servo1Pin = 12; // BEN: Right Servo
const int servo2Pin = 11; // BEN: Left Servo
...
void loop() {
  ...
  float ch6_norm = constrain((channel_6_pwm - 1100.0) / 800.0, 0.0, 1.0);
  if (ch6_norm > transition_fader)
  {
    transition_fader = transition_fader + dt/faderate;
  }
  else if ((ch6_norm < transition_fader))
  {
    transition_fader = transition_fader - dt/faderate;
  }
  else
  {
    transition_fader = ch6_norm;
  }
  transition_fader = constrain(transition_fader, 0.0, 1.0);
  //Serial.println(transition_fader);


  //PID Controller - SELECT ONE:
  // fade the gains
  Kp_roll_angle = (1.0 - transition_fader)*(kp_roll_angle_ff) + (transition_fader)*(kp_roll_angle_hover); // (1.0 - transition_fader)*(forward flight) + (transition_fader)*(hover)
  Kd_roll_angle = (1.0 - transition_fader)*(kd_roll_angle_ff) + (transition_fader)*(kd_roll_angle_hover);
  Kp_pitch_angle = (1.0 - transition_fader)*(kp_pitch_angle_ff) + (transition_fader)*(kp_pitch_angle_hover);
  Kd_pitch_angle = (1.0 - transition_fader)*(kd_pitch_angle_ff) + (transition_fader)*(kd_pitch_angle_hover);
  Kp_yaw = (1.0 - transition_fader)*(kp_yaw_ff) + (transition_fader)*(kp_yaw_hover);
  Kd_yaw = (1.0 - transition_fader)*(kd_yaw_ff) + (transition_fader)*(kd_yaw_hover);
...
void controlMixer() {
  ...
  m1_command_scaled = (1.0 - transition_fader)*(thro_des + yaw_PID) + (transition_fader)*(thro_des - roll_PID); // right
  m2_command_scaled = (1.0 - transition_fader)*(thro_des - yaw_PID) + (transition_fader)*(thro_des + roll_PID); // left

  //0.5 is centered servo, 0.0 is zero throttle if connecting to ESC for conventional PWM, 1.0 is max throttle
  s2_command_scaled = (1.0 - transition_fader)*(servo_left_trim + roll_PID + pitch_PID + kff_pitch_angle_ff*pitch_passthru) + (transition_fader)*(servo_left_trim + yaw_PID + pitch_PID); // left
  s1_command_scaled = (1.0 - transition_fader)*(servo_right_trim + roll_PID - pitch_PID - kff_pitch_angle_ff*pitch_passthru) + (transition_fader)*(servo_right_trim + yaw_PID - pitch_PID); // right
...
void Madgwick6DOF(float gx, float gy, float gz, float ax, float ay, float az, float invSampleFreq) {
  ...
  */
  ...
  roll_IMU = (1.0 - transition_fader)*(atan2(q0*q1 + q2*q3, 0.5f - q1*q1 - q2*q2)*57.29577951) + (transition_fader)*(asin(constrain(2.0f * (q0*q1 + q2*q3),-0.99999,0.99999))*57.29577951); // (1.0 - transition_fader)*(ff) + (transition_fader)*(hover)
  pitch_IMU = (1.0 - transition_fader)*(-asin(constrain(-2.0f * (q1*q3 - q0*q2),-0.99999,0.99999))*57.29577951) + (transition_fader)*(atan2(q1*q3 - q0*q2, 0.5f - (q1*q1 + q2*q2))*57.29577951+90.0f);
  yaw_IMU = (1.0 - transition_fader)*(-atan2(q1*q2 + q0*q3, 0.5f - q2*q2 - q3*q3)*57.29577951) + (transition_fader)*(-atan2(-q1*q2 + q0*q3, 0.5f - (q1*q1 + q3*q3))*57.29577951);
  float temp = GyroZ;
  GyroZ = (1.0 - transition_fader)*(GyroZ) + (transition_fader)*(GyroX);
  GyroX = (1.0 - transition_fader)*(GyroX) + (transition_fader)*(-temp);
}
...
void scaleCommands() {
  ...
  s2_command_PWM = s2_command_scaled*180; // BEN: Servo reverse
...
void getCommands() {
   ...
   */
    ...
    channel_1_pwm = getRadioPWM(3); // Throttle BEN
    channel_2_pwm = getRadioPWM(1); // Aileron
    channel_3_pwm = getRadioPWM(2); // Elevator
    channel_4_pwm = getRadioPWM(4); // Roll
